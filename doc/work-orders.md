...
## 11. 第五轮工单（2026-09-19 用户指令：「所有存在提示为开发中的功能都必须全部实现」，WO6 系列）

> 清点"开发中/即将上线"占位入口：编辑器模板按钮、编辑器分享按钮、首页社区/教程/定价导航（×3）、
> 首页升级套餐、编辑器左侧工具栏的录制/文本/品牌面板（help 面板为空）。本轮全部真实化。

| 编号 | 标题 | 优先级 | 状态 |
|---|---|---|---|
| WO6-01 | 模板中心真实化：编辑器「模板」按钮打开 TemplatesSideSheet（SideSheet 模板卡片列表），「使用模板」把 prompt 预填进 AI 对话（aiChatStore 新增 draft/setDraft）并切到 AI 面板 | P0 | ✅ 本次 |
| WO6-02 | 分享真实化：优先 navigator.share 系统分享面板，否则剪贴板复制项目链接（execCommand 兜底），取消分享不报错 | P0 | ✅ 本次 |
| WO6-03 | 社区页 `/community`：热门模板（复用 TemplateGrid）+ 热门作品（ProjectCard 网格） | P1 | ✅ 本次 |
| WO6-04 | 教程页 `/tutorials`：四篇入门教程（快速上手/AI 改剪/导出分享/录制素材，Collapse 展开）+ 创作入口；编辑器 help 面板接「打开使用教程」 | P1 | ✅ 本次 |
| WO6-05 | 定价页 `/pricing`：三档方案卡片（功能清单/价格/当前方案标记），升级走确认弹窗本地记录（演示环境不接支付，弹窗明示）；首页「升级套餐」接 /pricing | P1 | ✅ 本次 |
| WO6-06 | 首页导航 社区/教程/定价 接真实路由（公共路由，无需登录） | P1 | ✅ 本次 |
| WO6-07 | 录制面板真实化：MediaRecorder 摄像头+麦克风录制，实时取景与计时；完成后 blob 生成本地素材（playbackUrl=对象 URL，播放器可直接预览、可拖时间轴），真实模式同时 presign 直传上传；权限拒绝/不支持/上传失败均有 Toast；卸载释放设备 | P0 | ✅ 本次 |
| WO6-08 | 文本面板真实化：字幕内容+色板+位置+时长 → `timelineStore.addCaption` 在字幕轨播放头处建 caption 片段（轨道不存在自动创建、进撤销栈、自动选中），检查器「字幕」tab 可继续编辑文本 | P0 | ✅ 本次 |
| WO6-09 | 品牌面板真实化：brandStore（brandStore.ts，localStorage 持久化）品牌名/主色/辅色；主色作为文本面板字幕默认色；品牌名作为播放器右下角水印 | P1 | ✅ 本次 |
| WO6-10 | 全部新增用户可见文案走 i18n（zh/en 同步，约 70 键）；移除 MediaPanel 占位分支（toolLabels/「功能开发中」Empty）；废弃键按 WO4-10 决策保留不删 | P1 | ✅ 本次 |

**实施结果（2026-09-19）**：WO6-01~10 已实现并推送，验证交远端 CI。
**申报**：① 定价升级为本地记录（支付后端不在本期范围，弹窗已明示演示语义）；② 录制在演示模式
生成本地 blob 素材（刷新后丢失，真实模式上传持久化）；③ 模板数据沿用 mockTemplates（模板后端
API 不存在，属数据层限制，UI/交互为真实实现）。

---

## 12. 第六轮工单（2026-09-19 用户复核：「占位的地方要有真实的实现」，WO7 系列）

> 用户复核 WO6 后指出：不是把提示移除就行，占位处必须有真实实现。逐项复核后确认
> WO6 主体为真（录制/导入/页面/路由），但存在三处"交互真了但看不到效果"的空转，本轮补齐。

| 编号 | 标题 | 优先级 | 状态 |
|---|---|---|---|
| WO7-01 | 字幕预览真实渲染：播放器按片段 style（颜色/位置/字号）实时叠加显示字幕文本，不再是"加了字幕画面没变化" | P0 | ✅ 本次 |
| WO7-02 | 调色/滤镜/效果预览真实生效：activeClip 的 brightness/contrast/params.filter/effectType 映射为 CSS filter 实时作用于 `<video>`（最终成片以后端 B17 渲染管线为准） | P0 | ✅ 本次 |
| WO7-03 | 模板中心双真实路径：保留「使用模板」预填 AI 对话（真实模式走 Chat API 改剪），新增「用模板新建」带 prompt 进 Describe→Generate 真实生成链（Describe 已消费 ?prompt= 参数） | P1 | ✅ 本次 |

**WO6 真实性复核结论（2026-09-19）**：录制（MediaRecorder→素材→预览可播）、编辑器导入素材
（presign 直传）、分享（系统分享/剪贴板）、社区/教程/定价三页与路由为真实实现；定价升级为
本地记录（无支付后端，弹窗已明示）；模板/社区数据为演示数据（后端无对应 API，数据层限制）。
...
## 14. 第八轮工单（2026-09-19 用户指令：「没写完的功能都写完」，WO9 系列）

> 范围：账本登记的全部未完成功能项（WO5-14~17/19~20、WO3 后续登记的项目重命名/复制、
> 播放器逐帧、Timeline 完整虚拟滚动）。WO8-14~20 为健壮性债非功能项，保留待排期；
> WO5-18 死代码删除仍待人类。

| 编号 | 标题 | 状态 |
|---|---|---|
| WO9-01 | 项目重命名/复制：Projects 操作菜单新增"重命名"（Modal 改名，PATCH /projects/:id）与"复制项目"（新建同名副本 + 复制最新时间线；素材对象不复制，对象存储复制需后端支持已在工单申报）；mock 模式本地等效实现 | ✅ 本次 |
| WO9-02 | 上传中保留文件列表：进度面板替代全屏遮罩（步骤区禁点但可见），支持取消（已上传保留）与失败文件单独重试（复用已创建项目，单文件失败不中断整批） | ✅ 本次 |
| WO9-03 | Describe 模板入口矛盾修复：无 projectId 点生成时自动创建项目并走完整生成链（真实模式项目无素材时由 Generate API 明确报错——失败诚实）；表单重置按钮 + Ctrl/Cmd+Enter 快捷生成 | ✅ 本次 |
| WO9-04 | 首页：侧栏可折叠（收起/窄条展开按钮），搜索框全尺寸可见（小屏收窄而非隐藏，过滤能力不再消失） | ✅ 本次 |
| WO9-05 | 版本历史：回滚成功后 invalidate 版本列表（回滚即新版本）；SideSheet 宽度 min(380px, 90vw) 小屏自适应 | ✅ 本次 |
| WO9-06 | 播放头键盘可达：role="slider" + ←/→ ±0.5s（Shift ±1s）；canvas 定位改 ref 传入，移除类名字符串匹配的脆弱耦合 | ✅ 本次 |
| WO9-07 | 播放器逐帧步进：控制条 ←/→ 按钮 ±1/30s，与倍速共同补齐"倍速/逐帧"登记项 | ✅ 本次 |
| WO9-08 | Timeline 视口窗口化：Track 只渲染与可见窗口（±10s 缓冲）相交的片段，滚动动态换入换出，替代 200 条硬截断——所有片段滚到即可交互，DOM 数量与可见范围成正比 | ✅ 本次 |
| WO9-09 | 分析完成页摘要数字标注"演示数据"（真实聚合统计由后端任务产出前不做假数字） | ✅ 本次 |

**实施结果（2026-09-19）**：WO9-01~09 全部实现并推送，验证交远端 CI。至此账本登记的
功能类工单全部完成；剩余：WO8-14~20 健壮性 P2（待排期）、WO5-18 死代码删除（待人类）。
